### R Analysis of Lichen Bacterial Community 
...
biomfile=biomformat::read_biom("16S Lichen Kraken.biom2")

taxonomy_biom=biomformat::observation_metadata(biomfile)
taxonomy_biom

write.table(taxonomy_biom, 'taxonomy_16S_lichen_kraken.csv')
...
write.table(taxonomy_tree, sep=",", quote=FALSE, row.names= FALSE, fileEncoding="UTF-8", 'taxonomy_16S_lichen_tree.csv')
...
#perl .\taxonomy_to_tree.pl -b 60 -s "," -f .\taxonomy_16S_Lichen_tree.csv > tree_16S_lichen.nwk
#afterwards you have to open your file in notepad, and save as UTF-08

otu_biom=phyloseq::import_biom("16S Lichen Kraken.biom2")
write.csv(x = otu_table(otu_biom), file = "otu_table_16S_lichen_kraken.csv")

#reading files, you need four inputs: 
#otu table, taxonomy table, metadata

otu_table=read.csv("otu_table_16S_lichen_kraken.csv", row.names=1, header = T)
head(otu_table)
otu_table=as.matrix(otu_table)

taxonomy=read.csv("taxonomy_16S_lichen_kraken.csv", row.names=1, header = T)
replace(taxonomy, taxonomy == " ", NA) -> tax1
taxonomy1=as.matrix(taxonomy)

metadata=read.csv("metadata_16S_lichen_new.csv", row.names=1, header=T)
metadata
...
tre_fpath = "tree_lichen_betanti.nwk"
tree <- ape::read.tree(tre_fpath, comment.char = "")

#Amsterdam
com_Amsterdam <- read.csv("phyraresuball_asv_kraken.csv", header = TRUE, row.names = 1)
com_Amsterdam <- t(com_Amsterdam)
com_Amsterdam [1:5, 1:3]

#Amsterdam
match.phylo.comAmsterdam <- match.phylo.data(TRE, t(com_Amsterdam)); # species as rows, samples as columns for feature table
data_Amsterdam <- match.phylo.comAmsterdam$data
tree_Amsterdam <- match.phylo.comAmsterdam$phy
...
phy_rare <- rarefy_even_depth(physeqbac, sample.size = 50263, rngseed = 711, replace = F, trimOTUs = TRUE, 	verbose = F)
phy_rare

#phyloseq-class experiment-level object
#otu_table()   OTU Table:         [ 5053 taxa and 42 samples ]
#sample_data() Sample Data:       [ 42 samples by 17 sample variables ]
#tax_table()   Taxonomy Table:    [ 5053 taxa by 7 taxonomic ranks ] 
...
phy_rare <- subset_samples(phy_rare,!analysis_name %in% c("temperaturezone1_Xanthoria parietina_G", "temperaturezone1_Xanthoria parietina_J", "temperaturezone1_Candelaria concolor_A", "temperaturezone3_Physcia adscendens_H" ))
phy_rare
...
################ Phyrare sub sample Based on Temperature Zone/Urbanization Gradient #######################
...
richness_all_tempzone_facetgrid <- ggplot(data = alphadte.richness,
  ...
  expand_limits(y=200:1800) +
  scale_y_continuous(breaks=seq(200,1800, by=200)) +
...
Pairwise_Permanova_xanthoriaparietina <- pairwise.adonis(comxanthoriaparietina_tempzone, df_xanthoriaparietina$tempzone, sim.function = "vegdist", sim.method = "bray", perm = 999)
...
physeq_bnti_lichen=phyloseq(OTU, TAX, META, tree)
physeq_bnti_lichen

physeqbac_bnti_lichen <- physeq_bnti_lichen %>% subset_taxa(Kingdom == "Bacteria" &
                                                              Family != "Mitochondria" &
                                                              Class != "Chloroplast" &
                                                              Kingdom != "Archaea" &
                                                              Kingdom != "Unassigned")

get_taxa_unique(physeqbac_bnti_lichen,"Kingdom")
get_taxa_unique(physeqbac_bnti_lichen,"Family")
get_taxa_unique(physeqbac_bnti_lichen,"Class")

colnames(tax_table(physeqbac_bnti_lichen))
otu_table(physeqbac_bnti_lichen)[1:5, 1:54]
tax_table(physeqbac_bnti_lichen)[1:5, 1:7]
physeqbac_bnti_lichen

tax_table(physeqbac_bnti_lichen)[1:5, 1:7]
any(taxa_sums(physeqbac_bnti_lichen) < 1) #false
any(is.na(otu_table(physeqbac_bnti_lichen))) #false

# Make a data frame with a column for the read counts of each sample
sample_sum_df <- data.frame(sum = sample_sums(physeqbac_bnti_lichen))
sample_sum_df
...
# rarefy your dataset to lowest sequencing length, 711 should be kept constant:
phy_rare_bnti_lichen <- rarefy_even_depth(physeqbac_bnti_lichen, sample.size = 46199, rngseed = 711, replace = F, trimOTUs = TRUE, 	verbose = F)
phy_rare_bnti_lichen

ggrare(phy_rare_bnti_lichen, step = 100, color = "uhi_range", se = F)
sample_names(phy_rare_bnti_lichen)
sample_sums(phy_rare_bnti_lichen)

saveRDS(phy_rare_bnti_lichen, "phy_rare_lichen.rds")
otu_table_lichen_filt <- otu_table(phy_rare_bnti_lichen)[rowSums (otu_table(phy_rare_bnti_lichen)) > 20 & rowSums(otu_table(phy_rare_bnti_lichen) > 0) >= 3, ]

comm=t(data.frame(otu_table_lichen_filt))
group=data.frame(sample_data(phy_rare_bnti_lichen))
tree_betanti_lichen=phy_tree(phy_rare_bnti_lichen)
...
tax.ck=NST::match.name(cn.list = list(comm=comm), tree.list = list(tree=tree_betanti_lichen))
comm=tax.ck$comm
...
rc_data <- rc_data[-c(41), ]
write.csv(x = (rc_data), file = "rc_data.csv")
rc_data=read.csv("rc_data.csv", row.names=1, header = T)
...
plot_process_facetgrid <- ggarrange(plot_process_tempzone, plot_process_lichen, plot_process_tempzone, plot_process_lichen,
                                    align = "v", font.label = list(size = 14, face = "bold", family="serif"), 
                                    common.legend = FALSE, legend = "right", ncol = 2, nrow = 2)

plot_process_facetgrid
...
#there's 373 unique code

new_asv_code <- data.frame(Code=unique(c(Deseq_CP$Code,Deseq_CX$Code, Deseq_PX$Code)), 
                           ASV=paste0(paste0('B',seq(from = 1, to = 373)))) 
...
library(circlize)
pdf(file = 'Dendro.pdf', width = 38, height = 37)
...
draw(lgd_links, x = unit(0.1, "npc"), y = unit(900, "mm"), 
     just = c("right", "bottom"))
circos.clear()
dev.off()
...
df_successionall_significant_candelaria <- df_successionall_subset_candelaria %>% select(Group,OTU40, OTU49, OTU175, OTU212, OTU386, OTU453, OTU464, OTU466, OTU512, OTU532)
...
Heatmap_candelaria <- heatmap_candelaria_data[,1:10] # data
anno_one_candelaria <- heatmap_candelaria_data[,11] # annotation
anno_two_candelaria <- heatmap_candelaria_data[,12] # annotation
anno_one_candelaria <- factor(anno_one_candelaria, levels = c("temperaturezone1","temperaturezone2","temperaturezone3"))
annotation_col_candelaria = data.frame(
...
heatmap_candelaria_plot <- pheatmap::pheatmap(t(Heatmap_candelaria), 
                                              ...
                                              main = '', 
                                              fontsize = 5, 
                                              annotation_col = annotation_col_candelaria,
                                              gaps_row = c(3,6,9),
                                              treeheight_row = 100
)
...
write.csv(x = otu_table(phyraresub_physciaadscendens), file = "phyraresub_physciaadscendens_heatmap.csv")

#after you download, change the name of the column from code to temperature zone_tree_replicate

otu_table_physcia=read.csv("phyraresub_physciaadscendens_heatmap.csv", row.names=1, header = 1, check.names = F)
...
df_successionall_significant_physcia <- df_successionall_subset_physcia %>% select(Group,OTU40, OTU84, OTU109)
...
heatmap_physcia_data <- read.csv("df_heatmap_physcia.csv",
                                 header=T, row.names=1)
Heatmap_physcia <- heatmap_physcia_data[,1:3] # data
anno_one_physcia <- heatmap_physcia_data[,4] # annotation
anno_two_physcia <- heatmap_physcia_data[,5] # annotation
...
otu_table_xanthoria=read.csv("phyraresub_xanthoriaparietina_heatmap.csv", row.names=1, header = 1, check.names = F)
...
df_successionall_significant_xanthoria <- df_successionall_subset_xanthoria %>% select(Group,OTU623,OTU990,OTU991,OTU997,OTU1025,OTU1026,OTU1304,OTU1424,OTU1595,OTU2056,OTU2505,OTU3195)
...
heatmap_xanthoria_data <- read.csv("df_heatmap_xanthoria.csv",
                                   header=T, row.names=1)
Heatmap_xanthoria <- heatmap_xanthoria_data[,1:12] # data
anno_one_xanthoria <- heatmap_xanthoria_data[,13] # annotation
anno_two_xanthoria <- heatmap_xanthoria_data[,14] # annotation
...
#Taxonomy Table Heatmap ----------------
otu_table_heatmap=read.csv("phyraresuball_asv_kraken.csv", row.names=1, header = T)
...
class_colors <- c("Acidobacteriia" = "pink", "Actinomycetia" = "blue", "Alphaproteobacteria" = "lavender",
                  "Betaproteobacteria" = "purple", "Chitinophagia" = "red",
                  "Deltaproteobacteria" = "yellow", "Deinococci" = "beige", "Cyanophyceae" = "cyan",
                  "Flavobacteriia" = "skyblue", "Gammaproteobacteria" = "salmon", "Sphingobacteriia" = "brown", 
                  "Thermoleophilia" = "green", "TM7"= "skyblue", "Blastocatellia" = "violet",
                  "Unassigned" = "black")
...
plot_taxa_new <- ggarrange(m_treatment, m_treatment, m_treatment, m_treatment,align = "v", font.label = list(size = 14, face = "bold", family="serif"), 
                           common.legend = FALSE, legend = "right", ncol = 2, nrow = 2)
...
otu_df_candelaria <- otu_table(candelariaconcolor) %>% 
...
otu_annot_df_candelaria <- otu_df_candelaria %>%
...
####### Pie chart lichen bacteria ##########

function_bacteria_lichen=read.csv("deseq_bacteria_lichen.csv", row.names=1, header = T, sep=";")

sorted_unique_names <- function_bacteria_lichen %>% distinct(guildfunction) %>% arrange(guildfunction)
print(sorted_unique_names)

function_bacterialichen_group1_cp <- subset(function_bacteria_lichen, code == 'CP')
function_bacterialichen_group2_pc <- subset(function_bacteria_lichen, code == 'PC')
function_bacterialichen_group3_cx <- subset(function_bacteria_lichen, code == 'CX')
function_bacterialichen_group1_xc <- subset(function_bacteria_lichen, code == 'XC')
function_bacterialichen_group2_px <- subset(function_bacteria_lichen, code == 'PX')
function_bacterialichen_group3_xp <- subset(function_bacteria_lichen, code == 'XP')

preference_counts_group1_cp <- table(function_bacterialichen_group1_cp$guildfunction)
preference_counts_group2_pc <- table(function_bacterialichen_group2_pc$guildfunction)
preference_counts_group3_cx <- table(function_bacterialichen_group3_cx$guildfunction)
preference_counts_group1_xc <- table(function_bacterialichen_group1_xc$guildfunction)
preference_counts_group2_px <- table(function_bacterialichen_group2_px$guildfunction)
preference_counts_group3_xp <- table(function_bacterialichen_group3_xp$guildfunction)

sorted_preferences_group1_cp <- sort(preference_counts_group1_cp, decreasing = TRUE)
print(sorted_preferences_group1_cp)

sorted_preferences_group2_pc <- sort(preference_counts_group2_pc, decreasing = TRUE)
print(sorted_preferences_group2_pc)

sorted_preferences_group3_cx <- sort(preference_counts_group3_cx, decreasing = TRUE)
print(sorted_preferences_group3_cx)

sorted_preferences_group1_xc <- sort(preference_counts_group1_xc, decreasing = TRUE)
print(sorted_preferences_group1_xc)

sorted_preferences_group2_px <- sort(preference_counts_group2_px, decreasing = TRUE)
print(sorted_preferences_group2_px)

sorted_preferences_group3_xp <- sort(preference_counts_group3_xp, decreasing = TRUE)
print(sorted_preferences_group3_xp)
...
bacteriafunction_colors <- c("aerobic chemoheterotrophy_adipate degradation"="#DBFFD6",
                             ...
                             "aerobic chemoheterotrophy_xenobiotics degradation" = "cyan",
                             "aerobic chemoheterotrophy_plant growth promotion" = "#ffa2c5",
...
bacterialichen_colors_part2 <- c("aerobic chemoheterotrophy adipate degradation"="#DBFFD6",
                                 "aerobic chemoheterotrophy cellulolysis"="#DBFFD6",
                                 "aerobic chemoheterotrophy denitrification"="#DBFFD6",
                                 "aerobic chemoheterotrophy fermentation"="#DBFFD6",
                                 "aerobic chemoheterotrophy hydrocarbon degradation"="#DBFFD6",
                                 "aerobic chemoheterotrophy plant growth promotion" = "#DBFFD6",
                                 "aerobic chemoheterotrophy xenobiotics degradation" = "#DBFFD6",
                                 "aerobic chemoheterotrophy xylanolysis"= "#DBFFD6",
                                 "aerobic chemoheterotrophy nitrogen fixation" = "#DBFFD6",
                                 "aerobic chemoheterotrophy sulfur oxidation" = "#DBFFD6",
                                 "aerobic chemoheterotrophy unknown" = "#DBFFD6",
                                 "aerobic chemoheterotrophy ureolysis" = "#DBFFD6",
                                 "aerobic chemoheterotrophy xenobiotic degradation" = "#DBFFD6",
                                 "animal parasites or symbionts human pathogens" = "#E7E8E8",
                                 "animal parasites or symbionts unknown" = "#E7E8E8",
                                 "chemoheterotrophy arsenic detoxification" = "#ECD4FF",
                                 "chemoheterotrophy denitrification"= "#ECD4FF",
                                 "chemoheterotrophy methanol oxidation"= "#ECD4FF",
                                 "chemoheterotrophy ureolysis"= "#ECD4FF",
                                 "chemoheterotrophy nitrogen fixation"= "#ECD4FF",
                                 "chemoheterotrophy nitrification"= "#ECD4FF",
                                 "chemoheterotrophy unknown"= "#ECD4FF",
                                 "chemoheterotrophy ureolysis"= "#ECD4FF",
                                 "chemoheterotrophy iron reduction"= "#ECD4FF",
                                 "chemoautotrophy nitrification"= "cyan",
                                 "extremophilic iron reduction"= "#ffa2c5",
                                 "extremophilic unknown"= "#ffa2c5",
                                 "oxygenic photoautotrophy toxin producing cyanobacteria"= "gold",
                                 "oxygenic photoautotrophy nitrogen fixation"= "gold",
                                 "oxygenic photoautotrophy unknown"= "gold",
                                 "unknown unknown" = "beige")

piechart_bacteria=read.csv("piechart_bacteria_lichen.csv", row.names=1, header = T, sep=";")

piechart_group1_bac_cp <- subset(piechart_bacteria, code == 'CP')
piechart_group2_bac_pc <- subset(piechart_bacteria, code == 'PC')
piechart_group3_bac_cx <- subset(piechart_bacteria, code == 'CX')
piechart_group1_bac_xc <- subset(piechart_bacteria, code == 'XC')
piechart_group2_bac_px <- subset(piechart_bacteria, code == 'PX')
piechart_group3_bac_xp <- subset(piechart_bacteria, code == 'XP')
...
piechart_all <- ggplot(piechart_bacteria, aes(x="", fill=guild, y=abundance))+
  ...
  scale_fill_manual(values = bacterialichen_colors_part2) +
...
library(dplyr)
piechart_group1_try_cp <- piechart_group1_bac_cp %>% 
...
piechart_group2_try_pc <- piechart_group2_bac_pc %>% 
  mutate(end = 2 * pi * cumsum(abundance)/sum(abundance),
         start = lag(end, default = 0),
         middle = 0.5 * (start + end),
         hjust = ifelse(middle > pi, 1, 0),
...
piechart_group3_try_cx <- piechart_group3_bac_cx %>% 
  mutate(end = 2 * pi * cumsum(abundance)/sum(abundance),
         start = lag(end, default = 0),
         middle = 0.5 * (start + end),
         hjust = ifelse(middle > pi, 1, 0),
...
piechart_group1_try_xc <- piechart_group1_bac_xc %>% 
  mutate(end = 2 * pi * cumsum(abundance)/sum(abundance),
         start = lag(end, default = 0),
         middle = 0.5 * (start + end),
         hjust = ifelse(middle > pi, 1, 0),
...
piechart_group2_try_px <- piechart_group2_bac_px %>% 
  mutate(end = 2 * pi * cumsum(abundance)/sum(abundance),
         start = lag(end, default = 0),
         middle = 0.5 * (start + end),
         hjust = ifelse(middle > pi, 1, 0),
...
piechart_group3_try_xp <- piechart_group3_bac_xp %>% 
  mutate(end = 2 * pi * cumsum(abundance)/sum(abundance),
         start = lag(end, default = 0),
         middle = 0.5 * (start + end),
         hjust = ifelse(middle > pi, 1, 0),
...
# Barplot
piechart_one_baclichen_cp <- ggplot(piechart_group1_try_cp, aes(x="", fill=guild, y=abundance))+
  geom_bar(stat="identity", colour="white", linewidth=1, width=1)+
...
piechart_one_baclichen_cp

piechart_two_baclichen_pc <- ggplot(piechart_group2_try_pc, aes(x="", fill=guild, y=abundance))+
  geom_bar(stat="identity", colour="white", linewidth=1, width=1)+
  geom_text(aes(x = 1.5 * sin(middle), y = 1.5 * cos(middle), label = abundance,
...
piechart_two_baclichen_pc

piechart_three_baclichen_cx <- ggplot(piechart_group3_try_cx, aes(x="", fill=guild, y=abundance))+
  geom_bar(stat="identity", colour="white", linewidth=1, width=1)+
  geom_text(aes(x = 1.5 * sin(middle), y = 1.5 * cos(middle), label = abundance,
                hjust = hjust, vjust = vjust), size = 6) +
  coord_polar(theta = "y")+
...
piechart_three_baclichen_cx

piechart_one_baclichen_xc <- ggplot(piechart_group1_try_xc, aes(x="", fill=guild, y=abundance))+
  geom_bar(stat="identity", colour="white", linewidth=1, width=1)+
  geom_text(aes(x = 1.5 * sin(middle), y = 1.5 * cos(middle), label = abundance,
                hjust = hjust, vjust = vjust), size = 6) +
  coord_polar(theta = "y")+
...
piechart_one_baclichen_xc

piechart_two_baclichen_px <- ggplot(piechart_group2_try_px, aes(x="", fill=guild, y=abundance))+
  geom_bar(stat="identity", colour="white", linewidth=1, width=1)+
  geom_text(aes(x = 1.5 * sin(middle), y = 1.5 * cos(middle), label = abundance,
                hjust = hjust, vjust = vjust), size = 6) +
  coord_polar(theta = "y")+
...
piechart_two_baclichen_px

piechart_three_baclichen_xp <- ggplot(piechart_group3_try_xp, aes(x="", fill=guild, y=abundance))+
  geom_bar(stat="identity", colour="white", linewidth=1, width=1)+
  geom_text(aes(x = 1.5 * sin(middle), y = 1.5 * cos(middle), label = abundance,
                hjust = hjust, vjust = vjust), size = 6) +
  coord_polar(theta = "y")+
...
piechart_three_baclichen_xp

piechart_one_bac_xc <- ggplot(piechart_group1_bac_xc, aes(x="", fill=guild, y=abundance))+
  geom_bar(stat="identity", colour="white", linewidth=1, width=1)+
  geom_text(aes(label=abundance), position = position_stack(vjust = 0.5)) +
  coord_polar(theta = "y")+
  scale_fill_manual(values = bacterialichen_colors_part2) +
...
plot_piechart_bac <- ggarrange(piechart_one_baclichen_cp, piechart_two_baclichen_pc, piechart_three_baclichen_cx, piechart_one_baclichen_xc, piechart_two_baclichen_px, piechart_three_baclichen_xp,
                               align = "v", font.label = list(size = 12, face = "bold", family="serif"), ncol = 3, nrow = 2, 
